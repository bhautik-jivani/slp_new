...
def checkLoginVal(request):
    ...
    try:
        ...
        if ( check_password(contractor_password, contra.password) ):
            ...
            data = {"status": "success", "message":"Thank you", "data": "Login successfully...", "url":"/contractor/dashboard/", 'jwtToken':token}
...
def signout(request):
    try:
        ...
        try:
            ...
            data = {"status": "success", "message":"Bye...", "data": "You are loggedout...","url":"/contractor/signin/"}
            return redirect('/contractor/signin/')
            # return HttpResponse(json.dumps(data), content_type='application/json')
        except ObjectDoesNotExist:
            data = {"status": "warning", "message":"Try again...", "data": "You are not loggedout with other acount..."}
            return HttpResponse(json.dumps(data), content_type='application/json')
    except KeyError:
        return redirect('/contractor/signin/')
...
def dashboard(request):
    try:
        ...
        return render(request, 'contractor/dashboard.html',{'users': userModel, 'products': prodModel, 'jobCategories': jobCategories,'tasks':taskModel})
    except KeyError:
        return redirect('/contractor/signin/')
...
def set_password(request):
    ...
    if request.method == 'POST':
        try:
            ...
            resettoken.delete()
            data = {'title':'Thank you','message': 'Password setted successfully...', 'icon':'success', 'url':'/contractor/signin/'}
...
def reset_pass(request):
    if request.method == 'POST':
        ...
        try:
            ...
            data = {"title":'Link sent', "message":'Please check mail for reset password...', "icon":'success','url':'/contractor/signin/'}
            return render(request, 'contractor/login-page.html', data)

        except ObjectDoesNotExist:
            data = {'title':'Try again','message': 'Email not found...', 'icon':'error', 'url':'/contractor/signin/'}
            return render(request, 'contractor/login-page.html', data)